function animate_matrix_3d(A, overlay, vector)
    % Tim Moroney, 2021
    ...
    if ~isequal(size(A), [2 2]) && ~isequal(size(A), [3 2]) && ~isequal(size(A), [3, 3])
        error('A must be a 2x2, 3x2, or 3x3 matrix.')
    end
    ...
    if issquare
        ...
        if size(A, 1) < 3
            ...
            x = sym('x', [1, 6], 'real');
            R = [cos(x(6)) -sin(x(6)) 0; sin(x(6)) cos(x(6)) 0; 0 0 1] * [cos(x(5)) 0 sin(x(5)); 0 1 0; -sin(x(5)) 0 cos(x(5));] * [1 0 0; 0 cos(x(4)) -sin(x(4)); 0 sin(x(4)) cos(x(4));] * V * D;
            F = @(x) R(1)/x(1)^2 + R(2)/x(2)^2 + R(3)/x(3)^2 - 1;
            sol = fsolve(F ,[0 0])
        end
