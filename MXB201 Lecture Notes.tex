...
In order to convert an arbitrary basis to an orthonormal basis, we 
must develop a process that is generalisable to any basis.

In this process, we begin with a basis \(W\), and construct an orthogonal basis
\(V\). To do so, we choose a basis in \(W\) to be the reference vector, then
for the \(i\)th basis in \(V\) we subtract the projection onto the previous 
\end{document}
